...
public final class AlertSettingsPanel extends VBox {
    ...
    public AlertSettingsPanel(AppContext ctx) {
        ...
        pref.setConverter(new javafx.util.StringConverter<>() {
            @Override public String toString(AlertPreference p) {
                if (p == null) return "";
                return switch (p) {
                    case VISUAL_ONLY -> "Visual only";
                    case AUDIO_AND_VISUAL -> "Audio + visual";
                };
            }
            @Override public AlertPreference fromString(String s) { return null; }
        });
        ...
        dur.setConverter(new javafx.util.StringConverter<>() {
            @Override public String toString(AlertDuration d) {
                if (d == null) return "";
                return switch (d) {
                    case SEC_10 -> "10 seconds";
                    case SEC_30 -> "30 seconds";
                    case MIN_1 -> "1 minute";
                    case UNTIL_RESOLVED -> "Until resolved";
                };
            }
            @Override public AlertDuration fromString(String s) { return null; }
        });
